/******************************************************************************
* File Name : gtexload.c                         Ver : 1.00  Date : 2002-07-31
*
* Description :
*
...
static BOOL	 fread_image( FILE*, GStexture*, BITMAPFILEHEADER*, BITMAPINFOHEADER*, RGBQUAD* );
static BOOL	 fread_image_2bit( FILE*, GStexture*, BITMAPINFOHEADER*, RGBQUAD* );
static BOOL	 fread_image_4bit( FILE*, GStexture*, BITMAPINFOHEADER*, RGBQUAD* );
static BOOL	 fread_image_8bit( FILE*, GStexture*, BITMAPINFOHEADER*, RGBQUAD* );
static BOOL	 fread_image_24bit( FILE*, GStexture*, BITMAPINFOHEADER* );
static BOOL  fread_image_32bit( FILE*, GStexture*, BITMAPINFOHEADER* );
...
)
{
	...
	/* イメージデータをテクスチャに読み込む */
	if ( fread_image( fp, pTexture, &header, &info, palette ) == FALSE )
	{
		fclose( fp );
...
)
{
	...
	{
	case 1:		/* ２色ビットマップ			*/
		error = fread_image_2bit( fp, ptexture, pbfinf, palette );
		break;

	case 4:		/* １６色ビットマップ		*/
		error = fread_image_4bit( fp, ptexture, pbfinf, palette );
		break;

	case 8:		/* ２５６色ビットマップ		*/
		error = fread_image_8bit( fp, ptexture, pbfinf, palette );
		break;

	case 24:	/* ２４ビットビットマップ	*/
		error = fread_image_24bit( fp, ptexture, pbfinf );
		break;

	case 32:	/* ３２ビットビットマップ	*/
		error = fread_image_32bit( fp, ptexture, pbfinf );
		break;
	}
...
)
{
	...
	/* テクスチャイメージのロック */
	pteximage = (BYTE*)gsTextureLock( ptexture );
...
)
{
	...
	for ( y = 0; y < height; y++ )
	{
		...
		{
			/* ピクセルを読み込む */
			ptexel[2] = fgetc( fp );	/* Blue  */
			ptexel[1] = fgetc( fp );	/* Green */
			ptexel[0] = fgetc( fp );	/* Red   */
			ptexel[3] = 0xff;			/* Alpha */
