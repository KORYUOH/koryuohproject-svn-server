...
typedef	struct
{
	...
	int                  n;						/* ジョイスティック数       */
	LPDIRECTINPUTDEVICE2 idev[GJOYSTICK_MAX];	/* ジョイスティックデバイス */
...
static void cleanup_joystick_device( LPDIRECTINPUTDEVICE2 );
static GJOYSTATE read_joystick_device( LPDIRECTINPUTDEVICE2 );
static HRESULT set_property( LPDIRECTINPUTDEVICE2,REFGUID,DWORD,DWORD,DWORD );
static BOOL reacquire_input( LPDIRECTINPUTDEVICE2 );
static void read_joystick_axis( LPDIRECTINPUTDEVICE2, int*, int*, int* );
...
)
{
	...
	/* ジョイスティックの軸の状態を読み込む */
	read_joystick_axis( joy->idev[ joy_no ], x, y, z );
}
...
)
{
	...
	{
		OutputDebugString( "DirectInputCreate 5.0 FAILED" );
...
)
{
	...
	/* 新しいバージョンのDirectInput のキーボードの生成 */
	...
	/* ｘ軸の無効ゾーンの設定 */
	if ( set_property( pdev, DIPROP_DEADZONE, DIJOFS_X,
	...
	/* ｙ軸の無効ゾーンの設定 */
	if ( set_property( pdev, DIPROP_DEADZONE, DIJOFS_Y,
	                                           DIPH_BYOFFSET, 5000 ) != DI_OK )
	{
		OutputDebugString( "SetProperty(DIPH_DEADZONE) FAILED\n" );
...
(
	LPDIRECTINPUTDEVICE2	joy		/* DirectInputジョイスティックデバイス  */
)
{
	if ( joy != NULL )
	{
...
set_property
(
	LPDIRECTINPUTDEVICE2	pdev,	/* DirectInputデバイス          */
